...
struct resolver_entity* resolver_create_new_entity_for_var_node_custom_scope(struct resolver_process* process, struct node* var_node, void* private, struct resolver_scope* scope, int offset){
    ...
    // ch149: also stamp the var_data union variant so the read-side
    // resolver code that consults var_data.dtype sees the same type.
...
struct resolver_entity* resolver_create_new_entity_for_function_call(struct resolver_result* result, struct resolver_process* process, struct resolver_entity* left_operand_entity, void* private){
    ...
    // ch237: stamp the name so codegen can chase native-function
    // entries back to their registered callbacks.
    entity->name  = left_operand_entity->name;
    entity->func_call_data.arguments = vector_create(sizeof(struct node*));
    return entity;
}

// ch237: build a NATIVE_FUNCTION entity for a discovered native sym.
...
// ch256: search ONLY the current resolver scope for a variable by
// name. Used by the validator to catch same-scope redeclarations
// without false-positiving on shadowing in an outer scope.
...
struct resolver_entity* resolver_get_function_in_scope(struct resolver_result* result, struct resolver_process* resolver, const char* func_name, struct resolver_scope* scope){
    ...
    // ch237: fall back to native-function entities so identifier
    // lookups for `test` (etc.) succeed at codegen time.
    if(!entity){
...
struct resolver_entity* resolver_follow_identifier(struct resolver_process* resolver, struct node* node, struct resolver_result* result){
    ...
    // ch237: if the regular scope search came up empty, try the
    // native-function symbol table - sets `test()` etc. up so the
    // codegen-time dispatcher can hand off to the registered cb.
    if(!entity){
...
// ch129: decide merge flags for an array-bracket entity. Out-of-bounds
// (pointer arithmetic) or non-NUMBER index = standalone entity;
// constant in-range index = JUST_USE_OFFSET so the merge pass can
...
void resolver_array_bracket_set_flags(struct resolver_entity* bracket_entity, struct datatype* dtype, struct node* bracket_node, int index){
    if(!(dtype->flags & DATATYPE_FLAG_IS_ARRAY) || (int)array_brackets_count(dtype) <= index){
        bracket_entity->flags = RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_LEFT_ENTITY
                              | RESOLVER_ENTITY_FLAG_NO_MERGE_WITH_NEXT_ENTITY
...
// size to the remaining bracket suffix.
...
// ch135: classify the entity chain into emit-flags for codegen.
// Sets one of FIRST_ENTITY_PUSH_VALUE / FIRST_ENTITY_LOAD_TO_EBX and
// FINAL_INDIRECTION_REQUIRED_FOR_VALUE / DOES_GET_ADDRESS based on
// what's in the chain.
static void resolver_finalize_result_flags(struct resolver_process* resolver, struct resolver_result* result){
    (void)resolver;
    ...
    while(entity){
        ...
        if(entity->type == RESOLVER_ENTITY_TYPE_UNARY_GET_ADDRESS){
            ...
            // Book ships this as `& ~A | B`, which because of `|`
            // having lower precedence than `&` parses as
            // `& ~(A | B)`. Replicated verbatim.
    ...
    if((last_entity->dtype.flags & DATATYPE_FLAG_IS_ARRAY)
       ...
       && !(last_entity->flags & RESOLVER_ENTITY_FLAG_USES_ARRAY_BRACKETS)){
        // `char abc[50]; char* p = abc;` -> no final indirection.
        flags &= ~RESOLVER_RESULT_FLAG_FINAL_INDIRECTION_REQUIRED_FOR_VALUE;
...
// ch135: inherit scope/dtype/offset from the previous entity, then
// adjust the dtype for the specific unary op.
static void resolver_finalize_unary(struct resolver_process* resolver, struct resolver_result* result, struct resolver_entity* entity){
    (void)resolver; (void)result;
    struct resolver_entity* prev = entity->prev;
    if(!prev){
        return;
    }
    entity->scope  = prev->scope;
    entity->dtype  = prev->dtype;
    entity->offset = prev->offset;
    if(entity->type == RESOLVER_ENTITY_TYPE_UNARY_INDIRECTION){
        int depth = entity->indirection.depth;
        entity->dtype.pointer_depth -= depth;
...
static void resolver_finalize_last_entity(struct resolver_process* resolver, struct resolver_result* result){
    struct resolver_entity* last = resolver_result_peek(result);
    switch(last->type){
        case RESOLVER_ENTITY_TYPE_UNARY_INDIRECTION:
        case RESOLVER_ENTITY_TYPE_UNARY_GET_ADDRESS:
            resolver_finalize_unary(resolver, result, last);
            break;
    }
}
