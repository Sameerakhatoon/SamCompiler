...
// parses the index, eats `]`, builds BRACKET(inner), then EXPRESSION
// with op `[]`.
static void parse_for_array(struct history* history){
...
// ch91: `a, b` sequence operator.
static void parse_for_comma(struct history* history){
    token_next();   // eat `,`
