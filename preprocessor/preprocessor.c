...
void preprocessor_handle_token(struct compile_process* compiler, struct token* token)
{
    switch(token->type)
    {
        ...
        // ch200 deviation: until later chapters wire #define / #include /
        // expansion handling, the pass-through default keeps the rest of
        // the pipeline functional. See docs/200.
        default:
