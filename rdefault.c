...
// ch136: default resolver implementation. Wires up the
// resolver_callbacks so codegen can drive the resolver without
// supplying its own merge / make_private / set_result_base /
// new_array_entity / delete_* logic.

static struct resolver_default_entity_data* resolver_default_new_entity_data(void);
static void                                 resolver_default_entity_data_set_address(struct resolver_default_entity_data* entity_data, struct node* var_node, int offset, int flags);
...
// Format `ebp-4` / `ebp+4` (no brackets, the asm template adds them).
...
static struct resolver_default_entity_data* resolver_default_new_entity_data(void){
    return calloc(1, sizeof(struct resolver_default_entity_data));
}
...
struct resolver_entity* resolver_default_merge_entities(struct resolver_process* process, struct resolver_result* result, struct resolver_entity* left_entity, struct resolver_entity* right_entity){
    (void)resolver_default_merge_array_calculate_out_offset;
    int new_pos = left_entity->offset + right_entity->offset;
    return resolver_make_entity(process, result, &right_entity->dtype, left_entity->node,
...
struct resolver_process* resolver_default_new_process(struct compile_process* compiler){
    return resolver_new_process(compiler, &(struct resolver_callbacks){
