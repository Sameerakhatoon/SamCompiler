...
// ch96: fixup system. The parser (and later codegen) registers a fix
// callback for work that has to be deferred (forward refs, etc.) and
// calls fixups_resolve once everything is in flight. Each fixup also
// owns an end callback so private state can be freed in
// fixup_sys_free.
