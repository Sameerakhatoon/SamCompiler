...
int main(void){
    struct compile_process* cp = compile_process_create("${scratch}", "/tmp/sam_ch19_out", 0);
...
# between_brackets. ')' fires AFTER lex_finish_expression but the
# in-expression check still passes because we test in_expression at the
# time of token_create. The 4 inner tokens (50, +, 20, )) all carry BB.
assert_contains "$got" "count=5"      "5 tokens total"
assert_contains "$got" "BB [50+20)"   "buffer contains the raw inside-paren text"
pass
