...
// ch226: serialize a token to a buffer in C-source form so a freshly
// concatenated token stream can be re-lexed.
...
// ch226: render an entire token vector to a string then re-lex it so
// adjacent tokens collapse (e.g. concat of two IDs into one). Returns
// the new token vector.
